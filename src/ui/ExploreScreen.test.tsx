...
function enterSlot() {
  ...
  fireEvent.click(screen.getByRole('button', { name: /^第1档 (新游戏|继续游戏)$/ }));
}
...
test('从探索进入二当家战斗，再撤出回到探索', () => {
  render(<App />);
  enterSlot();
  for (const label of ['查看可用情报', '探听西域经书的传闻', '追查此线索', '前往黑风寨', '前往山门', '前往西仓', '抄录二当家独行时辰', '前往二当家偏房', '迎战二当家']) {
...
test('带回燕回身秘笈后可在镇上参悟并重返黑风寨', () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: /研读《燕回身》镖师手抄本/ }));
