  ...
  return (
    <main className="page result-page">
      ...
      <div className="page-content">
        ...
        {Object.keys(result.lost).length > 0 && <p className="intro">{result.success ? '撤离代价' : '失去物资'}：{Object.entries(result.lost).filter(([, count]) => count > 0).map(([id, count]) => `${id} ×${count}`).join('、')}</p>}
